...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			SetNextWindowPos(ImVec2(vp_size.x - NOTIFY_PADDING_X, (vp_size.y / 100.0f * 8.0f) + height), ImGuiCond_Always, ImVec2(1.0f, 0.0f));
			ImGui::SetNextWindowSizeConstraints(ImVec2(140.f, -1.f), ImVec2(INFINITY, -1.f));
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);

			// Here we render the toast content
			{
				PushTextWrapPos(vp_size.x / 3.0f); // We want to support multi-line text, this will wrap the text after 1/3 of the screen width
				...
				switch (current_toast->type)
				{
				case ImGuiToastType_Success:
					pImDraw->AddCircleFilled(
						ImVec2(windowPos.x + 40, windowPos.y + 18),
						6,
						ImColor(0, 255, 0),
						360
					);
					pImDraw->AddCircleFilled(
						ImVec2(windowPos.x + 40 + 20, windowPos.y + 18),
						6,
						ImColor(0, 255, 0),
						360
					);
					pImDraw->AddCircleFilled(
						ImVec2(windowPos.x + 40 + 40, windowPos.y + 18),
						6,
						ImColor(0, 255, 0),
						360
					);
